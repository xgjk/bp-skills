...
获取 `groupId`、`employeeId`、`employeeName`、`periodName`、`month`。若用户只给姓名，通过 `bp-data-viewer` 的 `search_group_by_name` 定位。若没有 `periodName`，通过 `bp-data-viewer` 的 `get_all_periods` 取 `status=1` 的启用周期。
...
python3 /home/node/.openclaw/skills/bp-report-assembler/scripts/monthly_report_api.py render_full_report \
  --group_id {groupId} --month {month} \
  --employee_name "{employeeName}" --period_name "{periodName}"
```
