...
- KR 级输出完整判断单元（衡量标准对照 + KR灯色 + 环比），目标级灯色从 KR 灯色聚合
-->

#### 2.1 目标清单总览

<!-- AI 指引（不输出）：仅列出参与自查的目标，被排除规则排除的目标不在此表中 -->

| 目标编号 | BP目标 | 本月承诺口径 | 本月实际 | 证据引用 | 目标灯色 | 结论一句话 |
|---------|--------|-------------|---------|----------|----------|------------|
| G1 | [目标全称] | [阈值/里程碑/交付物] | [达成情况] | [R{编号}](huibao://view?id={reportId}) | 🟢/🟡/🔴/⚫ | [一句话] |
| Gx | [目标全称] | [同上] | [同上] | [R{编号}](huibao://view?id={reportId}) | 🟢/🟡/🔴/⚫ | [同上] |

#### 2.2 未参与自查目标汇总

<!-- AI 指引（不输出）：若所有目标均参与自查，省略本节。若存在被排除的目标，以下表格列出 -->

以下目标不纳入本月自查：

| 目标编号 | BP目标 | 计划时间范围 | 排除原因 |
|---------|--------|-------------|---------|
| Gy | [目标全称] | [planDateRange 或 无] | [计划期未覆盖本月 / 草稿状态] |
...
[点击进入月度自评](https://sg-al-cwork-web.mediportal.com.cn/BP-manager/web/dist/#/monthly-review/self?groupId={groupId}&month={month})
...
[嵌入上月自评 Markdown：加权总分 + 各目标各维度评分表]
