...
若汇报的汇报内容为空为空或仅有格式标签无实质内容，该汇报不计入有效证据。
...
正文中所有证据引用使用**汇报直链**格式，点击直接跳转到对应汇报详情页。**正文只展示编号，不附带书名号标题**（标题统一在附录 A.2/A.3 中查看）：

- **当月证据（R 编号）**：`[R0301](huibao://view?id={reportId})`
  - R 后跟两位月份 + 两位序号
  - `{reportId}` 为该证据条目对应的原始 reportId
  - **正文不附带书名号标题**，标题仅出现在附录 A.2 证据清单中
- **上月参考（RP 编号）**：`[RP01](huibao://view?id={reportRecordId})`
  - RP 编号与 R 编号格式保持一致，**正文不附带书名号标题**
