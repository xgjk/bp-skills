...
- **判断卡片（Step 3c）**：仅对**参与自查**的关键成果和关键举措生成卡片并判灯（参与条件见下方排除规则）。
- **最终报告（Step 3d）**：
  - **第 2 章**：灯色判断到**目标级**——综合该目标下所有参与自查的 KR 卡片得出。若目标被排除规则整体排除，则不参与灯色统计，不展开自查明细。
- 所有灯色判断点使用统一的**三灯判断块**格式（见 report-template-bp-self-check.md）。
...
- 报告中在"未参与自查节点"汇总区做简要列示，注明排除原因（计划期未覆盖本月 / 草稿状态）
...
对**参与自查**的节点（计划区间与汇报月份有交集且非草稿），严格按以下优先级判灯，命中即停：

### 1. 先判黑灯 ⚫

当月该节点下**没有任何有效汇报证据**（按 evidence-rules.md 标准） → ⚫
...
**含义**：该关键成果/举措当月没有有效汇报证据，无法判断进展。（进入本步判断的节点已通过排除规则确认参与自查。）
