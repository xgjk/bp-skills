...
2. **内容聚合**：标题相同且同一天发送的汇报，经模型判断正文描述同一事项后合并为一个工作事项。**兜底规则：若无法确定是否为同一事项，默认不合并（宁可多不可少）。**
...
- **汇报正文**（从 `uniqueReportMap`）：对超长汇报（>4000字）执行 AI 摘要（控制在 500-800 字）。摘要必须保留所有数量/百分比/日期等关键数据点；若原文包含表格或列表，摘要中以精简列表保留
...
**严格按 [validation-rules.md](../rules/validation-rules.md) 第一章执行 15 项校验清单。** 全部通过后方可进入发送流程。
